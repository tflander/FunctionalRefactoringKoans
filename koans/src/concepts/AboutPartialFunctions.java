...
public class AboutPartialFunctions {
    ...
    public void pluggableStringConcatination() {
        ...
        assertEquals("foo bar", concatWithSpace.apply("foo", "bar"));
        ...
        // looks like this:
        assertEquals("foobar", applyOperatorToStrings.apply("foo", "bar").apply(concat));
        assertEquals("foo bar", applyOperatorToStrings.apply("foo", "bar").apply(concatWithSpace));
        ...
        assertEquals("foobar", applyToFooBar.apply(concat));
        assertEquals("foo bar", applyToFooBar.apply(concatWithSpace));
    }
    ...
    public void pluggableUsingPreferences() {
        ...
        assertEquals("foobar", concatKeyValue.apply("foo", preferences));
        assertEquals("foo: bar", showPrettyKeyValue.apply("foo", preferences));
        assertEquals("invalidnot found", concatKeyValue.apply("invalid", preferences));
        assertEquals("invalid: not found", showPrettyKeyValue.apply("invalid", preferences));
        ...
        // another function that takes a function that takes the parameter & preferences, then applies the function
        ...
        assertEquals("foobar", applyOperatorToStringUsingPreferences.apply(preferences).apply("foo").apply(concatKeyValue));
        assertEquals("foo: bar", applyOperatorToStringUsingPreferences.apply(preferences).apply("foo").apply(showPrettyKeyValue));
